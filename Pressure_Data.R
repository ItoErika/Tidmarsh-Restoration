...
#TW_PZ_01_Nov                       
Plot_Times<-as.POSIXct(TW_PZ_01_Nov[2:nrow(TW_PZ_01_Nov),"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_01_Nov[2:nrow(TW_PZ_01_Nov),], aes(Plot_Times, 166.3/100-(25.8/100+TW_PZ_01_Nov[2:nrow(TW_PZ_01_Nov),"m_water"])))+geom_line(color='royalblue3', size=.7) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_01")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%d %b")+ scale_y_reverse(limits =c(.6,0)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))
#TW_PZ_02_Nov
Plot_Times<-as.POSIXct(TW_PZ_02_Nov[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_02_Nov, aes(Plot_Times, 165/100-(19/100+TW_PZ_02_Nov[,"m_water"])))+geom_line(color='royalblue3', size=.5)+ ylim(0,.55) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_02")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%d %b")+theme(axis.text.x = element_text(angle=45, vjust = 0.5))  
#TW_PZ_03_Nov 
...
#TW_PZ_06_SAND_Nov
Plot_Times<-as.POSIXct(TW_PZ_06_SAND_Nov[2:nrow(TW_PZ_06_SAND_Nov),"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")

...
sim75flatBv = geostats.sisim(df,'X','Y','Facies',ivtype=0,koption=0,ncut=2,thresh=thresh,gcdf=gcdf,trend=dummy_trend,
               tmin=tmin,tmax=tmax,zmin=0.0,zmax=1.0,ltail=1,ltpar=1,middle=1,mpar=0,utail=1,utpar=1,
               nx=nx,xmn=xmn,xsiz=xsiz,ny=ny,ymn=ymn,ysiz=ysiz,seed = 64961,
               ndmin=ndmin,ndmax=ndmax,nodmax=nodmax,mults=1,nmult=3,noct=-1,radius=radius,ktype=0,vario=varios)
...
# Plot the simulations for the dxf conversion
plt.figure(figsize=(20*1.8395879323031639,1.2*1.8410852713178292))                                    # plot the results
fig=plt.imshow(simfl75at_exAv, cmap='Greys_r')
plt.axis('off')
fig.axes.get_xaxis().set_visible(False)
...
sim25hum1025A = geostats.sisim(df,'X','Y','Facies',ivtype=0,koption=0,ncut=2,thresh=thresh,gcdf=gcdf,trend=dummy_trend,
               tmin=tmin,tmax=tmax,zmin=0.0,zmax=1.0,ltail=1,ltpar=1,middle=1,mpar=0,utail=1,utpar=1,
               nx=nx,xmn=xmn,xsiz=xsiz,ny=ny,ymn=ymn,ysiz=ysiz,seed = 67307,
               ndmin=ndmin,ndmax=ndmax,nodmax=nodmax,mults=1,nmult=3,noct=-1,radius=radius,ktype=0,vario=varios)


sim25hum1025B = geostats.sisim(df,'X','Y','Facies',ivtype=0,koption=0,ncut=2,thresh=thresh,gcdf=gcdf,trend=dummy_trend,
               tmin=tmin,tmax=tmax,zmin=0.0,zmax=1.0,ltail=1,ltpar=1,middle=1,mpar=0,utail=1,utpar=1,
               nx=nx,xmn=xmn,xsiz=xsiz,ny=ny,ymn=ymn,ysiz=ysiz,seed = 65071,
               ndmin=ndmin,ndmax=ndmax,nodmax=nodmax,mults=1,nmult=3,noct=-1,radius=radius,ktype=0,vario=varios)

sim25hum1025C = geostats.sisim(df,'X','Y','Facies',ivtype=0,koption=0,ncut=2,thresh=thresh,gcdf=gcdf,trend=dummy_trend,
               tmin=tmin,tmax=tmax,zmin=0.0,zmax=1.0,ltail=1,ltpar=1,middle=1,mpar=0,utail=1,utpar=1,
               nx=nx,xmn=xmn,xsiz=xsiz,ny=ny,ymn=ymn,ysiz=ysiz,seed = 64969,
               ndmin=ndmin,ndmax=ndmax,nodmax=nodmax,mults=1,nmult=3,noct=-1,radius=radius,ktype=0,vario=varios)


# 64923, 64937, 67123, 67157
# 65051, 65039, 65031, 64969, 64929, 64919, 64917, 67383

# Expand the simulation by a factor of 4
sim25hum1025_exA = expand(sim25hum1025A,4)
sim25hum1025_exB = expand(sim25hum1025B,4)
sim25hum1025_exC = expand(sim25hum1025C,4)
...
bot_surf  = hum_h*np.cos((2/hum_p)*np.pi*dist)+10
...
sim25hum1025_exB2=sim25hum1025_exB
# Remove the area above hummocky top surface frpm the geostatistical simulations
for i in dist:
   sim25hum1025_exB2[0:60-top_surf_int[i],i]=np.nan
# Remove disturbed bottom surface
for i in dist:
   sim25hum1025_exB2[60-bot_surf_int[i]:60,i]=np.nan
   

# Plot the simulations for the dxf conversion
plt.figure(figsize=(20*1.8395879323031639,0.6*1.8410852713178292))                                    # plot the results
fig=plt.imshow(sim25hum1025_exA, cmap='Greys_r')
plt.axis('off')
fig.axes.get_xaxis().set_visible(False)
plt.savefig('sim25hum1025_ink_A.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

plt.figure(figsize=(20*1.8395879323031639,0.6*1.8410852713178292))                                    # plot the results
fig=plt.imshow(sim25hum1025_exB, cmap='Greys_r')
plt.axis('off')
fig.axes.get_xaxis().set_visible(False)
plt.savefig('sim25hum1025_ink_B.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

plt.figure(figsize=(20*1.8395879323031639,0.6*1.8410852713178292))                                    # plot the results
fig=plt.imshow(sim25hum1025_exC, cmap='Greys_r')
plt.axis('off')
fig.axes.get_xaxis().set_visible(False)
plt.savefig('sim25hum1025_ink_C.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

# Plot the simulations for potential figures
plt.figure(figsize=(20,0.60))                                    # plot the results
fig=plt.imshow(sim25hum1025_exA, cmap='Greys_r')
# Save as a png
plt.savefig('sim25hum1025_A.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

# Plot the simulation
plt.figure(figsize=(20,0.60))                                    # plot the results
fig=plt.imshow(sim50flat_exB, cmap='Greys_r')
# Save as a png
plt.savefig('sim25hum1025_B.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

# Plot the simulation
plt.figure(figsize=(20,0.60))                                    # plot the results
fig=plt.imshow(sim50flat_exC, cmap='Greys_r')
# Save as a png
plt.savefig('sim25hum1025_C.png', bbox_inches='tight', dpi=300, frameon='false', pad_inches=0)

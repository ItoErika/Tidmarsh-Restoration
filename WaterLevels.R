...
TW_PZ_05_Nov2018<-TW_PZ_05_Nov2018[which(TW_PZ_05_Nov2018[,"Date_Time"]=="06/19/18 03:15:00 PM"):which(TW_PZ_05_Nov2018[,"Date_Time"]=="11/19/18 04:00:00 PM"),]          
# Smooth the spikes in the data caused by logger removal during data downloading
# Interpolate between the 7/11 spike
...
TW_PZ_05_Nov2018[which(TW_PZ_05_Nov2018[,"Date_Time"]=="11/19/18 04:00:00 PM"),"m_manual"]<-0.114
# Save as CSV  
...
# Remove erroneous rows of data 
TW_PZ_05_Jun319<-TW_PZ_05_Jun319[which(TW_PZ_05_Jun319[,"Date_Time"]=="11/19/18 04:15:00 PM"):which(TW_PZ_05_Jun319[,"Date_Time"]=="06/03/19 11:00:00 AM"),]                                                 
# Add manual data
...
# Bind all of the data frames together...                  
TW_PZ_05_FULL<-rbind(TW_PZ_05_Nov2018, TW_PZ_05_Jun319)  
# Add tag in notes for where WL was recovering from isotope sampling
# Recovery from 7/11 sampling: 
TW_PZ_05_FULL[which(TW_PZ_05_FULL[,"Date_Time"]=="07/11/18 02:45:00 PM"):which(TW_PZ_05_FULL[,"Date_Time"]=="10/01/18 12:00:00 AM"),"Notes"]<-"sampling recovery"
# Recovery from 3/30/19 sampling
TW_PZ_05_FULL[which(TW_PZ_05_FULL[,"Date_Time"]=="03/30/19 03:30:00 PM"):which(TW_PZ_05_FULL[,"Date_Time"]=="05/01/19 12:00:00 AM"),"Notes"]<-"sampling recovery"
# Calculate mean height above GS and the standard deviation of the height above GS:  
# NOTE: we are ignoring the rows that are tagged with "sampling recovery" 
mean(na.omit(TW_PZ_05_FULL[-which(TW_PZ_05_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1212639 m
sd(na.omit(TW_PZ_05_FULL[-which(TW_PZ_05_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1065326 m  
# Save as CSV  
write.csv(TW_PZ_05_FULL, file="TWPZ05_FULL.csv", row.names=FALSE)  
...
# Add tag in notes for where WL was recovering from isotope sampling
# Not clear how WL in PZ was impacted by isotope sampling... there is a drop on 6/19, but there is also an increase around 6/23 and not sure why 
# Can't tell when recovery happened after sampling! So we will only use a bit of the data for the mean and SD calculation
# Recovery from 6/19 sampling: 
TW_PZ_05_SAND_FULL[which(TW_PZ_05_SAND_FULL[,"Date_Time"]=="06/19/18 03:15:00 PM"):which(TW_PZ_05_SAND_FULL[,"Date_Time"]=="11/19/18 03:45:00 PM"),"Notes"]<-"sampling recovery"
# Calculate mean height above GS and the standard deviation of the height above GS:  
# NOTE: we are ignoring the rows that are tagged with "sampling recovery" 
mean(na.omit(TW_PZ_05_SAND_FULL[-which(TW_PZ_05_SAND_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1190671 m
sd(na.omit(TW_PZ_05_SAND_FULL[-which(TW_PZ_05_SAND_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.04945013 m  
# Save as CSV  
write.csv(TW_PZ_05_SAND_FULL, file="TWPZ05SAND_FULL.csv", row.names=FALSE)    
                     
############################ TW_Grad_05 ############################  
# FULL
# Calculate the vertical gradient between PZ_05 and PZ_05_SAND     
# Join the pz and sw files together by the "Date_Time" column
TW_Grad_05_FULL<-join(TW_PZ_05_FULL[,c("Date_Time","m_below_GS", "m_manual", "Notes")], TW_PZ_05_SAND_FULL[,c("Date_Time","m_below_GS", "m_manual","Notes")], by="Date_Time")
# Rename the columns to designate between pz and sw water level values
colnames(TW_Grad_05_FULL)<-c( "Date_Time", "Peat_m_below_GS", "Peat_manual", "Peat_notes", "Sand_m_below_GS", "Sand_manual", "Sand_notes")
# Multiply m_below_GS columns by -1 to get head values with proper signage
TW_Grad_05_FULL[,"Peat_head_m"]<-TW_Grad_05_FULL[,"Peat_m_below_GS"]*-1
TW_Grad_05_FULL[,"Sand_head_m"]<-TW_Grad_05_FULL[,"Sand_m_below_GS"]*-1
# Multiply manual data by -1 as well
TW_Grad_05_FULL[,"Peat_manual"]<-TW_Grad_05_FULL[,"Peat_manual"]*-1
TW_Grad_05_FULL[,"Sand_manual"]<-TW_Grad_05_FULL[,"Sand_manual"]*-1
# Define dh (difference in head values)
TW_Grad_05_FULL[,"dh"]<-(TW_Grad_05_FULL[,"Peat_head_m"]-TW_Grad_05_FULL[,"Sand_head_m"])
# Define dz (vertical distance between midpoint of screens)
TW_Grad_05_FULL[,"dz"]<--1.232--.323         
# Calculate the gradient (dh/dz) such that a positive gradient indicates downward flow
TW_Grad_05_FULL[,"dh/dz"]<-TW_Grad_05_FULL[,"dh"]/TW_Grad_05_FULL[,"dz"] 
# Add the missing manual data into the data frame 
TW_Grad_05_FULL[which(TW_Grad_05_FULL[,"Date_Time"]=="11/19/18 03:00:00 PM"),"Sand_manual"]<--0.127
TW_Grad_05_FULL[which(TW_Grad_05_FULL[,"Date_Time"]=="03/30/19 01:30:00 PM"),"Sand_manual"]<--0.095
TW_Grad_05_FULL[which(TW_Grad_05_FULL[,"Date_Time"]=="06/03/19 11:00:00 AM"),"Sand_manual"]<--0.144
# Write CSV
write.csv(TW_Grad_05_FULL, file="TWGrad05_FULL.csv", row.names=FALSE)                

# Calculate the mean gradient
# Note: Ignore the sections of data where the WL is recovering from isotope sampling!
TW_Grad_05_FULL2<-subset(TW_Grad_05_FULL, is.na(TW_Grad_05_FULL[,"Peat_notes"])& is.na(TW_Grad_05_FULL[,"Sand_notes"]))
# Calculate average gradient
mean(na.omit(TW_Grad_05_FULL2[,"dh/dz"])) # NA
# Calculate standard deviation of gradient		       
sd(na.omit(TW_Grad_05_FULL2[,"dh/dz"])) # NA
...
# Add tag in notes for where WL was recovering from isotope sampling
# Recovery from 3/30/19 sampling
TW_PZ_06_FULL[which(TW_PZ_06_FULL[,"Date_Time"]=="03/30/19 03:15:00 PM"):which(TW_PZ_06_FULL[,"Date_Time"]=="04/01/19 12:00:00 AM"),"Notes"]<-"sampling recovery" 
...
# Save as CSV  
write.csv(TW_PZ_06_FULL, file="TWPZ06_FULL.csv", row.names=FALSE)     
#write.csv(TW_PZ_06_FULL_NO_ISO, file="TWPZ06_FULL_NO_ISO.csv", row.names=FALSE)     
# Calculate mean m below GS and the standard deviation of the m below GS:  
# NOTE: we are ignoring the rows that are tagged with "sampling recovery" 
mean(na.omit(TW_PZ_06_FULL[-which(TW_PZ_06_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1617731 m
sd(na.omit(TW_PZ_06_FULL[-which(TW_PZ_06_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1303057 m 
...
# Remove first and last few rows of data 
TW_PZ_06_SAND_Nov2018<-TW_PZ_06_SAND_Nov2018[which(TW_PZ_06_SAND_Nov2018[,"Date_Time"]=="06/18/18 02:45:00 PM"):which(TW_PZ_06_SAND_Nov2018[,"Date_Time"]=="11/19/18 04:00:00 PM"),]  
# Add manual data
...
# Add tag in notes for where WL was recovering from isotope sampling
# Recovery from 3/30/19 sampling
TW_PZ_06_SAND_FULL[which(TW_PZ_06_SAND_FULL[,"Date_Time"]=="03/30/19 03:15:00 PM"):which(TW_PZ_06_SAND_FULL[,"Date_Time"]=="06/03/19 10:45:00 AM"),"Notes"]<-"sampling recovery" 
# Save as CSV  
write.csv(TW_PZ_06_SAND_FULL, file="TWSW06SAND_FULL.csv", row.names=FALSE)    
# Calculate mean m below GS and the standard deviation of the m below GS:  
# NOTE: we are ignoring the rows that are tagged with "sampling recovery" 
mean(na.omit(TW_PZ_06_SAND_FULL[-which(TW_PZ_06_SAND_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.2115917 m
sd(na.omit(TW_PZ_06_SAND_FULL[-which(TW_PZ_06_SAND_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1554248 m  

############################ TW_Grad_06 ############################  
# FULL
# Calculate the vertical gradient between PZ_05 and PZ_05_SAND     
# Join the pz and sw files together by the "Date_Time" column
TW_Grad_06_FULL<-join(TW_PZ_06_FULL[,c("Date_Time","m_below_GS", "m_manual", "Notes")], TW_PZ_06_SAND_FULL[,c("Date_Time","m_below_GS", "m_manual","Notes")], by="Date_Time")
# Rename the columns to designate between pz and sw water level values
colnames(TW_Grad_06_FULL)<-c( "Date_Time", "Peat_m_below_GS", "Peat_manual", "Peat_notes", "Sand_m_below_GS", "Sand_manual", "Sand_notes")
# Multiply m_below_GS columns by -1 to get head values with proper signage
TW_Grad_06_FULL[,"Peat_head_m"]<-TW_Grad_06_FULL[,"Peat_m_below_GS"]*-1
TW_Grad_06_FULL[,"Sand_head_m"]<-TW_Grad_06_FULL[,"Sand_m_below_GS"]*-1
# Multiply manual data by -1 as well
TW_Grad_06_FULL[,"Peat_manual"]<-TW_Grad_06_FULL[,"Peat_manual"]*-1
TW_Grad_06_FULL[,"Sand_manual"]<-TW_Grad_06_FULL[,"Sand_manual"]*-1
# Define dh (difference in head values)
TW_Grad_06_FULL[,"dh"]<-(TW_Grad_06_FULL[,"Peat_head_m"]-TW_Grad_06_FULL[,"Sand_head_m"])
# Define dz (vertical distance between midpoint of screens)
TW_Grad_06_FULL[,"dz"]<--1.169--.354         
# Calculate the gradient (dh/dz) such that a positive gradient indicates downward flow
TW_Grad_06_FULL[,"dh/dz"]<-TW_Grad_06_FULL[,"dh"]/TW_Grad_06_FULL[,"dz"] 
# Write CSV
write.csv(TW_Grad_06_FULL, file="TWGrad06_FULL.csv", row.names=FALSE)                

# Calculate the mean gradient
# Note: Ignore the sections of data where the WL is recovering from isotope sampling!
TW_Grad_06_FULL2<-subset(TW_Grad_06_FULL, is.na(TW_Grad_05_FULL[,"Peat_notes"])& is.na(TW_Grad_06_FULL[,"Sand_notes"]))
# Calculate average gradient
mean(na.omit(TW_Grad_06_FULL2[,"dh/dz"])) # -0.1039163 m
# Calculate standard deviation of gradient		       
sd(na.omit(TW_Grad_06_FULL2[,"dh/dz"])) # 0.187551 m   
...
# Calculate mean height above GS and the standard deviation of the height above GS:  
mean(na.omit(TW_PZ_07_FULL[,"m_above_GS"])) # 0.2064886 m
sd(na.omit(TW_PZ_07_FULL[,"m_above_GS"])) # 0.05023819 m  
...
# Calculate mean height above GS and the standard deviation of the height above GS:  
mean(na.omit(TW_SW_07_FULL[,"m_above_GS"])) # 0.2107848 m
sd(na.omit(TW_SW_07_FULL[,"m_above_GS"])) # 0.04696841 m  
...
# Calculate the vertical gradient between PZ_07 and SW_07    
# Join the pz and sw files together by the "Date_Time" column
TW_Grad_07_FULL<-join(TW_PZ_07_FULL[,c("Date_Time","m_above_GS", "m_manual")], TW_SW_07_FULL[,c("Date_Time","m_above_GS", "m_manual")], by="Date_Time")
# Rename the columns to designate between pz and sw water level values
colnames(TW_Grad_07_FULL)<-c( "Date_Time", "PZ_m_above_GS", "PZ_manual", "SW_m_above_GS", "SW_manual")
# Define dh (difference in head values)
TW_Grad_07_FULL[,"dh"]<--(TW_Grad_07_FULL[,"PZ_m_above_GS"]-TW_Grad_07_FULL[,"SW_m_above_GS"])
# Define dz (vertical distance between midpoint of screens)
TW_Grad_07_FULL[,"dz"]<-1.1765-0         
# Calculate the gradient (dh/dz) such that a positive gradient indicates downward flow
...
TW_PZ_08_Jun2018<-TW_PZ_08_Jun2018[which(TW_PZ_08_Jun2018[,"Date_Time"]=="05/06/18 05:00:00 PM"):which(TW_PZ_08_Jun2018[,"Date_Time"]=="06/18/18 02:45:00 PM"),]    
# Add manual data
TW_PZ_08_Jun2018[which(TW_PZ_08_Jun2018[,"Date_Time"]=="05/06/18 05:00:00 PM"),"m_manual"]<-0.163       
...
TW_PZ_08_Nov2018[which(TW_PZ_08_Nov2018[,"Date_Time"]=="11/19/18 04:45:00 PM"),"m_manual"]<-0.114
...
# Remove the first few rows of erroneous data
TW_PZ_08_Jun319<-TW_PZ_08_Jun319[which(TW_PZ_08_Jun319[,"Date_Time"]=="11/19/18 05:00:00 PM"):which(TW_PZ_08_Jun319[,"Date_Time"]=="06/03/19 09:00:00 AM"),]    
# Add manual data   
...
# Add tag in notes for where WL was recovering from isotope sampling
# Recovery from 3/30 sampling: 
TW_PZ_08_FULL[which(TW_PZ_08_FULL[,"Date_Time"]=="03/30/19 02:30:00 PM"):which(TW_PZ_08_FULL[,"Date_Time"]=="06/03/19 09:00:00 AM"),"Notes"]<-"sampling recovery"
# Save as CSV  
write.csv(TW_PZ_08_FULL, file="TWPZ08_FULL.csv", row.names=FALSE)  
# Calculate mean m below GS and the standard deviation of the m below GS:  
# NOTE: we are ignoring the rows that are tagged with "sampling recovery" 
mean(na.omit(TW_PZ_08_FULL[-which(TW_PZ_08_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1206612 m
sd(na.omit(TW_PZ_08_FULL[-which(TW_PZ_08_FULL[,"Notes"]=="sampling recovery"),"m_below_GS"])) # 0.1283676 m 		       
		    
############################ TW_PZ_08_SAND ############################ 

# Create a column for the depth to water below ground surface
# The top of piezometer casing to ground surface = 110.7 cm
TW_PZ_08_SAND_Jun319[,"m_below_GS"]<-1.61-(110.7/100+TW_PZ_08_SAND_Jun319[,"m_water"])
# Remove the first few rows of erroneous data
TW_PZ_08_SAND_Jun319<-TW_PZ_08_SAND_Jun319[1:which(TW_PZ_08_SAND_Jun319[,"Date_Time"]=="06/03/19 09:00:00 AM"),]    	
...
TW_PZ_08_SAND_Jun319[which(TW_PZ_08_SAND_Jun319[,"Date_Time"]=="11/19/18 04:00:00 PM"),"m_manual"]<-0.018
# Save as CSV
write.csv(TW_PZ_08_SAND_Jun319, file="TWPZ08_SAND_11-19-18_to_6-3-19.csv", row.names=FALSE)    

# Bind all of the data frames together...                  		       
# Save as CSV
TW_PZ_08_SAND_FULL<-TW_PZ_08_SAND_Jun319
write.csv(TW_PZ_08_SAND_FULL, file="TWPZ08_SAND_FULL.csv", row.names=FALSE)   
# Calculate mean m below GS and the standard deviation of the m below GS:  
mean(na.omit(TW_PZ_08_SAND_FULL[,"m_below_GS"])) # 0.1362788 m
sd(na.omit(TW_PZ_08_SAND_FULL[,"m_below_GS"])) # 0.09234964 m  

############################ TW_Grad_08 ############################	       
		       
# FULL
# Calculate the vertical gradient between PZ_08 and PZ_08_SAND     
# Join the pz and sw files together by the "Date_Time" column
TW_Grad_08_FULL<-join(TW_PZ_08_FULL[,c("Date_Time","m_below_GS", "m_manual", "Notes")], TW_PZ_08_SAND_FULL[,c("Date_Time","m_below_GS", "m_manual")], by="Date_Time")
# Rename the columns to designate between pz and sw water level values
colnames(TW_Grad_08_FULL)<-c( "Date_Time", "Peat_m_below_GS", "Peat_manual", "Peat_notes", "Sand_m_below_GS", "Sand_manual")
# Multiply m_below_GS columns by -1 to get head values with proper signage
TW_Grad_08_FULL[,"Peat_head_m"]<-TW_Grad_08_FULL[,"Peat_m_below_GS"]*-1
TW_Grad_08_FULL[,"Sand_head_m"]<-TW_Grad_08_FULL[,"Sand_m_below_GS"]*-1
# Multiply manual data by -1 as well
TW_Grad_08_FULL[,"Peat_manual"]<-TW_Grad_08_FULL[,"Peat_manual"]*-1
TW_Grad_08_FULL[,"Sand_manual"]<-TW_Grad_08_FULL[,"Sand_manual"]*-1
# Define dh (difference in head values)
TW_Grad_08_FULL[,"dh"]<-(TW_Grad_08_FULL[,"Peat_head_m"]-TW_Grad_08_FULL[,"Sand_head_m"])
# Define dz (vertical distance between midpoint of screens)
TW_Grad_08_FULL[,"dz"]<--1.219--.268        
# Calculate the gradient (dh/dz) such that a positive gradient indicates downward flow
TW_Grad_08_FULL[,"dh/dz"]<-TW_Grad_08_FULL[,"dh"]/TW_Grad_08_FULL[,"dz"] 
# Write CSV
write.csv(TW_Grad_08_FULL, file="TWGrad08_FULL.csv", row.names=FALSE)  

# Calculate the mean gradient
# Note: Ignore the sections of data where the WL is recovering from isotope sampling!
TW_Grad_08_FULL2<-subset(TW_Grad_08_FULL, is.na(TW_Grad_08_FULL[,"Peat_notes"]))
# Calculate average gradient
mean(na.omit(TW_Grad_08_FULL2[,"dh/dz"])) # -0.06031218 m
# Calculate standard deviation of gradient		       
sd(na.omit(TW_Grad_08_FULL2[,"dh/dz"])) # 0.1275146 m                  
...
# Create a column for the depth to water below ground surface
# The top of piezometer casing to ground surface = 28 cm
TW_PZ_09_Jun2018[,"m_below_GS"]<-139.9/100-(28/100+TW_PZ_09_Jun2018[,"m_water"])
# Assign NA where the logger was extracted and logged erroneous data on 3/19
TW_PZ_09_Jun2018[which(TW_PZ_09_Jun2018[,"Date_Time"]=="03/19/18 10:15:00 AM"),"m_below_GS"]<-NA
# Add manual data
# NOTE: there is a big increase in the water column right after this. This is likely because we added 3 temperature sensors to the PZ at this time
TW_PZ_09_Jun2018[which(TW_PZ_09_Jun2018[,"Date_Time"]=="03/19/18 10:00:00 AM"),"m_manual"]<-0.198
# Save as CSV
...
# Bind all of the data frames together... 
# First need to remove the columns that make the dataframes uneven:       
TW_PZ_09_Nov2018<-TW_PZ_09_Nov2018[,-which(colnames(TW_PZ_09_Nov2018)=="NA.4")]
TW_PZ_09_Jun319<-TW_PZ_09_Jun319[,-which(colnames(TW_PZ_09_Jun319)=="NA.4")]
TW_PZ_09_FULL<-rbind(TW_PZ_09_Jun2018, TW_PZ_09_Nov2018, TW_PZ_09_Jun319)    
TW_PZ_09_FULL[which(TW_PZ_09_FULL[,"Date_Time"]=="03/19/18 10:00:00 AM"),"Notes"]<-"added 3 temp loggers"
write.csv(TW_PZ_09_FULL, file="TWPZ09_FULL.csv", row.names=FALSE) 
...
# Plot the data! 
ggplot() + geom_line(data = PZ4, aes(x, y, color='PZ'), size=0.3) + geom_line(data = SW4, aes(x, y, color='SW'), size=0.3) + geom_point(data = PZ4_man, aes(x, y, color='PZ manual'), size=3)+ geom_point(data = SW4_man, aes(x, y, color='SW manual'), shape=17, size=3)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-0.7, 0.7), breaks = seq(-1, 1, by = 0.1))+ ggtitle("PZ/SW_04")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SW_04_FULL.pdf", width = 12, height = 6) 
...
# Complete PZ_01 dataset
Plot_Times<-as.POSIXct(TW_PZ_01_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_05_FULL, aes(Plot_Times, TW_PZ_05_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.3) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_05")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+ scale_y_reverse(limits =c(.9,-.4)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))
ggsave("TW_PZ_05_FULL.pdf", width = 12, height = 6)                             
# Complete with manual
Plot_Times<-as.POSIXct(TW_PZ_05_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_05_FULL, aes(Plot_Times, TW_PZ_05_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.3) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_05")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+ scale_y_reverse(limits =c(.9,-.4)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+ geom_point(aes(x=Plot_Times, y=TW_PZ_05_FULL[,"m_manual"]), color="orange3", size=3) 
ggsave("TW_PZ_05_FULL_manual.pdf", width = 12, height = 6)                             
...
# Full dataset
Plot_Times<-as.POSIXct(TW_Grad_05_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_05_FULL, aes(Plot_Times, TW_Grad_05_FULL[,"dh/dz"]))+geom_point(color='royalblue3', size=0.3) + xlab("Date") + ylab("Vertical Hydraulic Gradient")+ scale_y_reverse(limits =c(0.4,-0.4), breaks=(seq(-.4,0.4,0.1)))+ ggtitle("TW_Gradient_05")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+theme(axis.text.x = element_text(angle=45, vjust = 0.5))
ggsave("TW_Grad_05_FULL.pdf", width = 12, height = 6)                        

# Plot head measurements together
# Complete with manual
Plot_Times<-as.POSIXct(TW_Grad_05_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_05_FULL, aes(Plot_Times))+geom_line(aes(y=Peat_head_m, colour='Peat'), size=0.3)+geom_line(aes(y=Sand_head_m, colour='Sand'), size=0.3)+ geom_point(aes(y=Peat_manual, color='Peat manual'), size=3)+ geom_point(aes(y=Sand_manual, color='Sand manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-1, 0.4), breaks = seq(-1, 0.5, by=0.1)) + ggtitle("PEAT/SAND 05")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SAND_05_FULL.pdf", width = 12, height = 6) 
# Complete with manual - ZOOM
Plot_Times<-as.POSIXct(TW_Grad_05_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_05_FULL, aes(Plot_Times))+geom_line(aes(y=Peat_head_m, colour='Peat'), size=0.3)+geom_line(aes(y=Sand_head_m, colour='Sand'), size=0.3)+ geom_point(aes(y=Peat_manual, color='Peat manual'), size=3)+ geom_point(aes(y=Sand_manual, color='Sand manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-0.8, 0.3), breaks = seq(-0.8, 0.3, by = 0.1)) + ggtitle("PEAT/SAND 05")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SAND_05_FULL_ZOOM.pdf", width = 12, height = 6) 
...
# Full dataset
Plot_Times<-as.POSIXct(TW_Grad_06_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_06_FULL, aes(Plot_Times, TW_Grad_06_FULL[,"dh/dz"]))+geom_point(color='royalblue3', size=0.3) + xlab("Date") + ylab("Vertical Hydraulic Gradient")+ scale_y_reverse(limits =c(0.7,-0.8), breaks=(seq(-1,1,0.1)))+ ggtitle("TW_Gradient_06")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+theme(axis.text.x = element_text(angle=45, vjust = 0.5))
ggsave("TW_Grad_06_FULL.pdf", width = 12, height = 6)                        

# Plot head measurements together
# Complete with manual
Plot_Times<-as.POSIXct(TW_Grad_06_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_06_FULL, aes(Plot_Times))+geom_line(aes(y=Peat_head_m, colour='Peat'), size=0.3)+geom_line(aes(y=Sand_head_m, colour='Sand'), size=0.3)+ geom_point(aes(y=Peat_manual, color='Peat manual'), size=3)+ geom_point(aes(y=Sand_manual, color='Sand manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-1, 0.8), breaks = seq(-1, 0.8, by = 0.5)) + ggtitle("PEAT/SAND 06")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SAND_06_FULL.pdf", width = 12, height = 6) 
# Complete with manual - ZOOM
Plot_Times<-as.POSIXct(TW_Grad_06_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_06_FULL, aes(Plot_Times))+geom_line(aes(y=Peat_head_m, colour='Peat'), size=0.3)+geom_line(aes(y=Sand_head_m, colour='Sand'), size=0.3)+ geom_point(aes(y=Peat_manual, color='Peat manual'), size=3)+ geom_point(aes(y=Sand_manual, color='Sand manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-0.8, 0.3), breaks = seq(-0.8, 0.3, by = 0.1)) + ggtitle("PEAT/SAND 06")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SAND_06_FULL_ZOOM.pdf", width = 12, height = 6) 
...
ggplot(TW_Grad_07_FULL, aes(Plot_Times, TW_Grad_07_FULL[,"dh/dz"]))+geom_point(color='royalblue3', size=0.3) + xlab("Date") + ylab("Vertical Hydraulic Gradient")+ scale_y_reverse(limits =c(.1,-.1), breaks=(seq(-.1,.1,.1)))+ ggtitle("TW_Gradient_07")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+theme(axis.text.x = element_text(angle=45, vjust = 0.5))
ggsave("TW_Grad_07_FULL.pdf", width = 12, height = 6)                        
.
# Plot head measurements together
# Complete with manual
Plot_Times<-as.POSIXct(TW_Grad_07_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_07_FULL, aes(Plot_Times))+geom_line(aes(y=PZ_m_above_GS, colour='PZ'), size=0.3)+geom_line(aes(y=SW_m_above_GS, colour='SW'), size=0.3)+ geom_point(aes(y=PZ_manual, color='PZ manual'), size=3)+ geom_point(aes(y=SW_manual, color='SW manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(0, 0.4), breaks = seq(-0.2, 0.4, by = 0.1)) + ggtitle("PZ/SW 07")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
...
ggplot(TW_PZ_08_SAND_Jun319, aes(Plot_Times, TW_PZ_08_SAND_Jun319[,"m_below_GS"]))+geom_line(color='royalblue3', size=.6) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_08_SAND")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%b %d")+ scale_y_reverse(limits =c(0.6,-0.2)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))                                                                                                      
ggsave("TW_PZ_08_SAND_11-19-18_to_6-3-19.pdf", width = 12, height = 6) 
# June with manual
Plot_Times<-as.POSIXct(TW_PZ_08_SAND_Jun319[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_08_SAND_Jun319, aes(Plot_Times, TW_PZ_08_SAND_Jun319[,"m_below_GS"]))+geom_line(color='royalblue3', size=.6) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_08_SAND")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%b %d")+ scale_y_reverse(limits =c(0.6,-0.2)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))   + geom_point(aes(x=Plot_Times, y=TW_PZ_08_SAND_Jun319[,"m_manual"]), color="orange3", size=3)                                                                                                                         
ggsave("TW_PZ_08_SAND_11-19-18_to_6-3-19_manual.pdf", width = 12, height = 6)                 

# Plot full dataset
Plot_Times<-as.POSIXct(TW_PZ_08_SAND_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_08_SAND_FULL, aes(Plot_Times, TW_PZ_08_SAND_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.6) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_08_SAND")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%b %d")+ scale_y_reverse(limits =c(0.6,-0.2)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))                                                                                                      
ggsave("TW_PZ_08_SAND_FULL.pdf", width = 12, height = 6) 
# Complete with manual
Plot_Times<-as.POSIXct(TW_PZ_08_SAND_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_08_SAND_FULL, aes(Plot_Times, TW_PZ_08_SAND_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.6) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_08_SAND")+  scale_x_datetime(breaks = seq(Plot_Times[1], Plot_Times[length(Plot_Times)], "7 days"),date_labels="%b %d")+ scale_y_reverse(limits =c(0.6,-0.2)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))   + geom_point(aes(x=Plot_Times, y=TW_PZ_08_SAND_FULL[,"m_manual"]), color="orange3", size=3)                                                                                                                         
ggsave("TW_PZ_08_SAND_FULL_manual.pdf", width = 12, height = 6) 
          

############################ TW_Grad_08 ############################   
# Full dataset
Plot_Times<-as.POSIXct(TW_Grad_08_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_08_FULL, aes(Plot_Times, TW_Grad_08_FULL[,"dh/dz"]))+geom_point(color='royalblue3', size=0.3) + xlab("Date") + ylab("Vertical Hydraulic Gradient")+ scale_y_reverse(limits =c(0.8,-0.5), breaks=(seq(-0.5,0.8,.1)))+ ggtitle("TW_Gradient_08")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+theme(axis.text.x = element_text(angle=45, vjust = 0.5))
ggsave("TW_Grad_08_FULL.pdf", width = 12, height = 6)                        

# Plot head measurements together
# Complete with manual
Plot_Times<-as.POSIXct(TW_Grad_08_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_Grad_08_FULL, aes(Plot_Times))+geom_line(aes(y=Peat_head_m, colour='Peat'), size=0.3)+geom_line(aes(y=Sand_head_m, colour='Sand'), size=0.3)+ geom_point(aes(y=Peat_manual, color='Peat manual'), size=3)+ geom_point(aes(y=Sand_manual, color='Sand manual'), size=3, shape=17)+ xlab("Date")+ ylab("Head (m)")+ scale_y_continuous(limits = c(-0.8, 0.4), breaks = seq(-0.8, 0.4, by = 0.1)) + ggtitle("Peat/Sand 08")+ scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y") +theme(axis.text.x = element_text(angle=45, vjust = 0.5))+scale_colour_manual(values=c("#bf812d","#543005","#5ab4ac","#003c30"))
ggsave("TW_PZ-SW_08_FULL.pdf", width = 12, height = 6) 	      
...
# Complete PZ_06 dataset    
Plot_Times<-as.POSIXct(TW_PZ_09_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_09_FULL, aes(Plot_Times, TW_PZ_09_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.3) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_09")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+ scale_y_reverse(limits =c(.9,-.1)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))                                           
ggsave("TW_PZ_09_FULL.pdf", width = 12, height = 6)                                                                     
# Complete with manual   
Plot_Times<-as.POSIXct(TW_PZ_09_FULL[,"Date_Time"], "%m/%d/%y %I:%M:%S %p", tz="America/New_York")
ggplot(TW_PZ_09_FULL, aes(Plot_Times, TW_PZ_09_FULL[,"m_below_GS"]))+geom_line(color='royalblue3', size=.3) + xlab("Date") + ylab("Depth to Water Below Ground Surface (m)")+ggtitle("TW_PZ_09")+  scale_x_datetime(breaks = seq(as.POSIXct("2018-05-01 00:00:00 EDT"), as.POSIXct("2019-09-01 00:00:00 EDT"), "1 month"),date_labels="%b %d, %Y")+ scale_y_reverse(limits =c(.9,-.1)) +theme(axis.text.x = element_text(angle=45, vjust = 0.5))  + geom_point(aes(x=Plot_Times, y=TW_PZ_09_FULL[,"m_manual"]), color="orange3", size=3)                                            
ggsave("TW_PZ_09_FULL_manual.pdf", width = 12, height = 6)             
##################################################################      
############################ TW_SW_10 ############################
##################################################################

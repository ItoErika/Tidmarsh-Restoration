...
# Create a vector of each autocorrelation coefficient for lags 0-9
Autocorrelation<-c(acv0/acv0, acv1/acv0, acv2/acv0, acv3/acv0, acv4/acv0, acv5/acv0, acv6/acv0, acv7/acv0, acv8/acv0, acv9/acv0)                    
# Create a vector of lags 0-9
Lag<-0:9  
# Calculate the 95% confidence bounds for the plot               
conf<-1.96/sqrt(length(MoistureData[cm_Rows,"cm_Soil_Moisture_Calculated_(%)"]))
# Create an autocorrelation plot 
plot(Lag, AutoCorrelation, abline(h=conf, lty=3), ylim=c(0,1), main="West (cm) Autocorrelation Plot")                
...
Lag<-0:9  
# Calculate the 95% confidence bounds for the plot               
conf<-1.96/sqrt(length(MoistureData[m_Rows,"m_Soil_Moisture_Calculated_(%)"]))
# Create an autocorrelation plot 
plot(Lag, AutoCorrelation, abline(h=conf, lty=3), ylim=c(0,1), main="West (m) Autocorrelation Plot")               
######################################################### Old Script ###################################################################
